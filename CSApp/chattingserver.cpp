...
void ChatServerForm::clientConnect( )                                               //고객이 연결되면 실행되는 슬롯함수
{
    QTcpSocket *clientConnection = chatServer->nextPendingConnection( );            //클라이언트 소켓 생성
    connect(clientConnection, SIGNAL(readyRead( )), SLOT(receiveData( )));          //read 할 준비가 되면 receiveData 실행
    connect(clientConnection, SIGNAL(disconnected( )), SLOT(removeClient()));       //연결이 끊어지면 removeClient 실행
    qDebug("new connection is established...");                                     //새로운 연결이 성공했다는 메세지 출력
}

void ChatServerForm::receiveData( )                                                 //클라이언트 소켓에서 데이터를 받는 슬롯함수
{
    QTcpSocket *clientConnection = dynamic_cast<QTcpSocket *>(sender( ));           //클라이언트의 소켓을 센더로 설정
    QByteArray bytearray = clientConnection->read(BLOCK_SIZE);                      //클라이언트 소켓의 데이터를 read할 byteArray 생성

    Chat_Status type;       // 채팅의 목적
    char data[1020];        // 전송되는 메시지/데이터
    memset(data, 0, 1020);

    QDataStream in(&bytearray, QIODevice::ReadOnly);                                //byteArray를 읽기전용으로 in이라는 변수에 담음
    in.device()->seek(0);                                                           //표준입력 0 = 키보드, QI/ODevice에 0을 대입해서 in의 표준 입력장치로 input Device를 설정
    in >> type;                                                                     //in에 type 데이터 출력
    in.readRawData(data, 1020);                                                     //in에 있는 데이터를 1020만큼 read해서 data에 저장

    QString ip = clientConnection->peerAddress().toString();                        //ip를 저장
    quint16 port = clientConnection->peerPort();                                    //port를 저장
    QString name = QString::fromStdString(data);                                    //name을 저장

    qDebug() << ip << " : " << type;                                                //현재 ip 출력
