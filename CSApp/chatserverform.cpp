...
void ChatServerForm::addClient(QList<int> cIdInfo, QList<QString> cNameInfo)
{
    int cnt = 0;
...
void ChatServerForm::modifyClient(int id, QString name)
{
...
void ChatServerForm::receiveData()
{
    ...
    case Chat_Talk: {
        ...
        break;
    case Chat_Out:
        foreach(auto item, ui->chattingRoomTreeWidget->findItems(name, Qt::MatchFixedString, 1)) {
            if(item->text(1) != name + " ") {
                item->setIcon(0, QIcon(":/icon_image/greenLight.png"));
                item->setText(1, name + " ");
            }
...
void ChatServerForm::inviteClient()
{
    ...
    if(ui->waittingRoomTreeWidget->currentItem()->text(1) != name + " ")    return;
...
void ChatServerForm::on_waittingRoomTreeWidget_customContextMenuRequested(const QPoint &pos)
{
    ...
    foreach(QAction *action, menu->actions()) {
        if(action->objectName() == "Invite")
            ...
            //현재 아이템의 텍스트가 이름과 같으면 invite 활성화
        else
...
void ChatServerForm::on_chattingRoomTreeWidget_customContextMenuRequested(const QPoint &pos)
{
    ...
    QString name = ui->chattingRoomTreeWidget->currentItem()->text(1);
    foreach(QAction *action, menu->actions()) {
        if(action->objectName() == "Kick out")
            action->setEnabled(ui->chattingRoomTreeWidget->currentItem()->text(1) == name);
            //현재 아이템의 텍스트가 이름과 같으면 Kick out 활성화
        else
            action->setDisabled(1);
    }
    QPoint globalPos = ui->chattingRoomTreeWidget->mapToGlobal(pos);
    menu->exec(globalPos);

}

void ChatServerForm::on_resetPushButton_clicked()
{
